...
//Rare exceptions to indicate a bug in the _postexchange code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_postexchange = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_postexchange]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.postexchange.module"
